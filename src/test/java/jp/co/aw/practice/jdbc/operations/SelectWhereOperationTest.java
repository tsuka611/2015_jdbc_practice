...
public class SelectWhereOperationTest {

    Connection connection;
    EmployeeService employeeService;
    SelectWhereOperation operation;

    @Before
    public void setUp() throws Exception {
        employeeService = new EmployeeService();
        operation = SelectWhereOperation.builder().employeeService(employeeService).build();
    ...
    @Test
    public void execute_取得結果がない() throws Exception {
        long id01 = insert(connection, "テスト太郎", "mail_taro@example.com", null, parse("2015/04/10 11:22:33"), false);
        findById(connection, id01);
        long id02 = insert(connection, "テスト二郎", "mail_jiro@example.com", null, parse("2015/04/11 11:22:33"), false);
        findById(connection, id02);
        ...
        StringBuilder input = new StringBuilder()//
                .append("ほげほげ").append(newline());
        StringBuilder expected = new StringBuilder()//
                .append("検索したい名前を入力してください").append(newline())//
                .append("[ほげほげ]ではレコードが選択されませんでした。").append(newline());
        StringReader r = new StringReader(input.toString());
        StringWriter w = new StringWriter();
        ConsoleWrapper c = new ConsoleWrapper(r, w);
        int ret = operation.execute(c);
